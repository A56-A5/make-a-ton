			...
			if messageText != "" {
				...
				if v.Info.IsFromMe {
					if strings.Contains(messageText, "Your order has been placed!") {
						// Identify Partner's Name for the invoice
						partnerName := partnerPhone // Fallback to phone
						partnerContact, _ := client.Store.Contacts.GetContact(context.Background(), v.Info.Chat)
						if partnerContact.FullName != "" {
							partnerName = partnerContact.FullName
						} else if partnerContact.PushName != "" {
							partnerName = partnerContact.PushName
						}

						// Send history to invoice service directly
						go sendHistoryToInvoice(partnerPhone, partnerName, currentHistory)
...
func sendToBackend(sender, message string, history []string) {
	backendURL := "http://localhost:5000/api/messages"
...
func sendHistoryToInvoice(customerPhone string, customerName string, chats []string) {
	invoiceURL := "http://localhost:5000/api/generate-invoice"
